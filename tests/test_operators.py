...
def test_symmetric(a: float, b: float) -> None:
    ...
    """
    assert_close(mul(a, b) - mul(b, a), 0)
...
def test_distribute(a: float, b: float, c: float) -> None:
    ...
    """
    assert_close(mul(a, b + c) - mul(a, b) - mul(a, c), 0)
...
def test_other(a: float, b: float, c: float) -> None:
    ...
    """
    assert_close(mul(a, mul(b, c)) - mul(mul(a, b), c), 0)
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ...
    for x in ls1:
        for y in ls2:
            result += mul(x, y)
    assert_close(mul(sum(ls1), sum(ls2)), result)
